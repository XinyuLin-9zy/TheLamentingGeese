...
namespace Utage
{
	...
	internal class SoundAudio : MonoBehaviour
	{
		...
		{
			if (this.Player == null || this.Player.Group == null || Data == null)
			{
				return null;
			}

			var group = this.Player.Group;
			if (group.SoundManager == null || group.SoundManager.TaggedMasterVolumes == null)
			{
		...
		{
			if (Data == null || AudioSource == null) yield break;
		...
		void SetNextIntroLoop()
		{
			...
			if (AudioSourceForIntroLoop == null) return;
		...
		void UpdatePlay()
		{
			if (Data == null || AudioSource == null || AudioSource.clip == null)
			{
				EndFadeOut();
				return;
			}
