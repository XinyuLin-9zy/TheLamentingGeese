...
namespace Utage
{
	...
	public class AssetBundleInfoManager : MonoBehaviour
	{
		...
		public float ManifestProgress { get; private set; }
...
//			Debug.Log("Load Start " + url);
			...
			BeginManifestLoading(url);
			yield return wwwEx.LoadAssetBundleByNameAsync<AssetBundleManifest>(
			...
			EndManifestLoading(isSucceeded);
		}

		void OnDownloadingManifest(WWWEx wwwEx)
		{
			ManifestProgress = Mathf.Clamp01(wwwEx.Progress);
//			Debug.Log(wwwEx.Progress);
		...
		void BeginManifestLoading(string url)
		{
			ManifestUrl = url;
			ManifestProgress = 0;
		...
		void EndManifestLoading(bool isSucceeded)
		{
			if (isSucceeded)
			{
				ManifestProgress = 1;
