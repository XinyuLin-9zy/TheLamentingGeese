...
namespace Utage
{
		...
		, IAdvSaveDelete
	{
		...
		protected virtual bool ReadSaveData(AdvSaveData saveData)
		{
			...
			{
				Exception exception;
				bool result = FileIOManager.TryReadBinaryDecode(saveData.Path, saveData.Read, out exception);
				if (result) return true;

				saveData.Clear();
				if (exception != null)
				{
					Debug.LogWarningFormat("Ignored unreadable save file '{0}'. The slot was reset and the corrupt file was removed. {1}: {2}",
						saveData.Path, exception.GetType().Name, exception.Message);
					DeleteUnreadableSaveDataFile(saveData.Path);
				}
				return false;
			}
			else
		...
		protected virtual void DeleteUnreadableSaveDataFile(string path)
		{
			try
			{
			...
			{
				Debug.LogWarningFormat("Failed to remove unreadable save file '{0}'. {1}: {2}", path, e.GetType().Name, e.Message);
			}
		}
