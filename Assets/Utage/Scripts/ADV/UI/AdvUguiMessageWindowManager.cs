...
namespace Utage
{
	...
	public class AdvUguiMessageWindowManager : MonoBehaviour, IAdvMessageWindowManager
	{
		...
		protected virtual void EnsureRenderOrder()
		{
			if (this.transform.parent == null) return;

			// Reconstructed scenes can put DialogMsg after this manager, hiding message text.
			Transform dialogMsg = this.transform.parent.Find("DialogMsg");
			if (dialogMsg == null) return;
			if (this.transform.GetSiblingIndex() < dialogMsg.GetSiblingIndex())
			{
				this.transform.SetSiblingIndex(dialogMsg.GetSiblingIndex());
			}
		}
	}
}
