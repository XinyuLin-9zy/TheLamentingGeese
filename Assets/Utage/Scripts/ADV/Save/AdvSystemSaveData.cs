...
namespace Utage
{
		...
		,IAdvSaveDelete
	{
		...
		protected virtual bool TryReadSaveData()
		{
			...
			bool result = FileIOManager.TryReadBinaryDecode(Path, ReadBinary, out exception);
			if (result) return true;

			if (exception != null)
			{
				readData = new AdvReadHistorySaveData();
				selectionData = new AdvSelectedHistorySaveData();
				galleryData = new AdvGallerySaveData();
				Debug.LogWarningFormat("Ignored unreadable system save file '{0}'. Runtime system data was reset and the corrupt file was removed. {1}: {2}",
					Path, exception.GetType().Name, exception.Message);
				DeleteUnreadableSystemSaveDataFile(Path);
			}
			return false;
		}

		protected virtual void DeleteUnreadableSystemSaveDataFile(string path)
		{
			try
			{
			...
			{
				Debug.LogWarningFormat("Failed to remove unreadable system save file '{0}'. {1}: {2}", path, e.GetType().Name, e.Message);
			}
		}
