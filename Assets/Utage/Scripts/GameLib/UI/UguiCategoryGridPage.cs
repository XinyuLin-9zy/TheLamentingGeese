...
namespace Utage
{
	...
	public class UguiCategoryGridPage : MonoBehaviour
	{
		...
		protected virtual void CreateTabButton(GameObject go, int index)
		{
			...
			if (image != null)
			{
				if (buttonSpriteList != null && index < buttonSpriteList.Count && buttonSpriteList[index] != null)
				{
					image.enabled = true;
					image.sprite = buttonSpriteList[index];
				}
			...
			{
				toggle.targetGraphic = image != null && image.enabled ? image : go.GetComponentInChildren<Graphic>(true);
			}
		}
