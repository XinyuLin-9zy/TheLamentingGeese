...
namespace Utage
{
	...
	public class AdvUguiLoadGraphicFile : MonoBehaviour
	{
		...
		public virtual void LoadTextureFile(string path)
		{
			...
			File.Unuse(this);
			++loadTextureRequestId;
			StartSafeCoroutine(CoWaitTextureFileLoading(File, loadTextureRequestId));
		}

		protected virtual IEnumerator CoWaitTextureFileLoading(AssetFile file, int requestId)
		{
			while (file != null && !file.IsLoadEnd) yield return null;
			if (this == null || gameObject == null) yield break;
			if (requestId != loadTextureRequestId || file == null || File != file) yield break;
		...
		protected virtual void StartSafeCoroutine(IEnumerator coroutine)
		{
		...
		protected virtual T ChangeGraphicComponent<T>() where T : Graphic
		{
			...
			//まだ設定されてないならGetする
			if (GraphicComponent == null || !GraphicComponent)
			{
		...
		protected class CoroutineRunner : MonoBehaviour
		{
			public static CoroutineRunner Instance
			{
				get
				{
					...
					{
						GameObject go = new GameObject("__AdvUguiLoadGraphicFileCoroutineRunner");
