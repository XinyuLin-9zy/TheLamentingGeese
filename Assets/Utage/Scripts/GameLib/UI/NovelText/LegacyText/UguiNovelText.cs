...
namespace Utage
{
		...
		, INovelText
	{
		...
		internal static Font GetFallbackFont()
		{
			...
			try
			{
				fallbackFont = Font.CreateDynamicFontFromOSFont(
					new[] { "Microsoft YaHei", "SimHei", "Noto Sans CJK SC", "Source Han Serif CN", "Arial", "Helvetica" }, 16);
			}
			catch
			{
				fallbackFont = null;
		...
		internal Font GetEffectiveFont()
		{
			EnsureFont();
			return font;
		}
		...
		internal void EnsureFont()
		{
			...
			if (font == null)
			{
				Debug.LogError("UguiNovelText font is null and no fallback font could be created.", this);
			}
		}
