...
namespace Utage
{
	...
	public class AdvUguiManager : AdvUiManager
	{
		...
		protected virtual void Update()
		{
			...
			bool isPointerInput = IsPointerAdvanceInput();
			switch (Status)
			{
				case UiStatus.Backlog:
					break;
				case UiStatus.HideMessageWindow:	//メッセージウィンドウが非表示
					//右クリック
					if (InputUtil.IsInputGuiClose() || isPointerInput)
					{	//通常画面に復帰
				...
				case UiStatus.Default:
					...
					{	//入力待ち
						...
						{
							if (isInput || (isPointerInput && IsShowingMessageWindow))
							{
								//メッセージ送り
								CancelAutoModeByManualInput();
		...
		protected virtual bool IsPointerAdvanceInput()
		{
			if (!InputUtil.EnableInput) return false;

			if (Input.GetMouseButtonDown(0))
			{
				return !IsPointerOverInteractiveControl(Input.mousePosition, -1);
			}
			...
			for (int i = 0; i < Input.touchCount; ++i)
			{
				...
				if (!IsPointerOverInteractiveControl(touch.position, touch.fingerId))
				{
					return true;
				}
			}
		...
		protected virtual bool IsPointerOverInteractiveControl(Vector2 screenPosition, int pointerId)
		{
			...
			{
				if (IsInteractivePointerTarget(result.gameObject))
				{
					return true;
				}
			}
		...
		protected virtual bool IsInteractivePointerTarget(GameObject target)
		{
			...
			return scrollRect != null && scrollRect.isActiveAndEnabled;
		}
		...
		protected virtual void CancelAutoModeByManualInput()
		{
			if (Engine != null && Engine.Config != null && Engine.Config.IsAutoBrPage)
			{
				Engine.Config.IsAutoBrPage = false;
			}
		}
	}
}
