...
namespace Utage
{
	...
	public partial class AssetFileManager : MonoBehaviour
	{
		...
		static public void InitLoadTypeSetting(AssetFileManagerSettings.LoadType loadTypeSetting)
		{
			...
			if (instance.Settings == null || instance.Settings.FileSettings == null || instance.Settings.FileSettings.Count == 0)
				instance.Settings = CreateDefaultSettings();
			instance.Settings.BootInit(loadTypeSetting);
		}
		...
		public static AssetFileManager GetInstance()
		{
			if (instance == null)
			{
				...
				if (instance == null)
				{
					instance = WrapperFindObject.FindObjectOfTypeIncludeInactive<AssetFileManager>();
				}
				if (instance == null)
				{
					GameObject go = new GameObject("AssetFileManager");
		...
		static AssetFileManagerSettings CreateDefaultSettings()
		{
			var s = new AssetFileManagerSettings();
			s.AddExtensions(AssetFileType.Text, new string[]{".txt",".json",".html",".htm",".xml",".yaml",".fnt",".bin",".bytes",".csv",".tsv"});
			s.AddExtensions(AssetFileType.Texture, new string[]{".png",".jpg",".bmp",".psd",".tif",".tga",".gif",".iff",".pict"});
			s.AddExtensions(AssetFileType.Sound, new string[]{".mp3",".ogg",".wav",".aif",".aiff",".xm",".mod",".it",".s3m"});
			s.BootInit(AssetFileManagerSettings.LoadType.Local);
			return s;
		}
	}
}
