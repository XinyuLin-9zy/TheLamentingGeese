...
namespace Utage
{
	...
	public class AdvUguiMessageWindow : MonoBehaviour, IAdvMessageWindow, IAdvMessageWindowCaracterCountChecker
	{
		...
		protected virtual void PrepareLegacyTextRendering()
		{
			if (text == null || textPro != null) return;

			TMPro.TMP_Text tmpText = text.GetComponent<TMPro.TMP_Text>();
			if (tmpText != null)
			{
				if (text.fontSize < 20 && tmpText.fontSize > text.fontSize)
				{
					text.fontSize = Mathf.RoundToInt(tmpText.fontSize);
				}
				tmpText.enabled = false;
			}

			foreach (var behaviour in text.GetComponents<Behaviour>())
			{
				if (behaviour == null || behaviour == text) continue;
				string typeName = behaviour.GetType().FullName;
				if (typeName == "Febucci.UI.TextAnimator_TMP" || typeName == "Utage.TextMeshProRuby")
				{
