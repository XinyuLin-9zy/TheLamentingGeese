...
namespace Utage
{
	...
	public class UguiCategoryPanel : MonoBehaviour
	{
			...
			if (categoryList.Length > 1)
			{
				IEnumerable<Toggle> CreateToggles()
				{
					for (int i = 0; i < categoryList.Length; i++)
					{
						...
						PrepareCategoryTab(go, categoryList[i]);
		...
		protected virtual void PrepareCategoryTab(GameObject go, string category)
		{
			if (go == null) return;

			string localized = LanguageManager.Instance != null
				? LanguageManager.Instance.LocalizeText(category)
				: category;
			TextComponentWrapper.SetTextInChildren(go, localized);
			...
			{
				toggle.targetGraphic = go.GetComponentsInChildren<Graphic>(true).FirstOrDefault(x => x != null && x.enabled);
			}
		}
