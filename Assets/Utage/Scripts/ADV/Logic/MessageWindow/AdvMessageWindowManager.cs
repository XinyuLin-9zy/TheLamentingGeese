...
namespace Utage
{
	...
	public class AdvMessageWindowManager : MonoBehaviour, IAdvSaveData
	{
		...
		internal void ChangeCurrentWindow(string name)
		{
			...
			else
			{
				...
				if (!ActiveWindows.TryGetValue(name, out window))
				{
					...
					if (!AllWindows.TryGetValue(name, out window))
					{
						// Reconstructed scenario data can contain legacy WindowType values
						// that no longer match the imported UI object names.
						if (!TryGetDefaultWindow(out name, out window))
						{
							return;
						}
					}
		...
		bool TryGetDefaultWindow(out string name, out AdvMessageWindow window)
		{
			name = "";
			window = null;
			...
			return AllWindows.TryGetValue(name, out window);
		}
