...
namespace Utage
{
	...
	public class LanguageData
	{
		...
		internal bool TryLocalizeText( out string text, string currentLanguage, string defaultLanguage, string key, string dataName = "")
		{
			...
			if (TryGetFallbackText(out text, key)) return true;
			return false;
		}
		...
		bool TryGetFallbackText(out string text, string key)
		{
			text = "";
			if (TryGetNonEmptyText(out text, "English", key)) return true;
			if (TryGetNonEmptyText(out text, "Japanese", key)) return true;

			if (!dataTbl.TryGetValue(key, out LanguageStrings strings))
			{
				return false;
			}

			foreach (string str in strings.Strings)
			{
		...
		bool TryGetNonEmptyText(out string text, string language, string key)
		{
			if (TryGetText(out text, language, key) && !string.IsNullOrEmpty(text))
			{
				return true;
			}
			return false;
