...
namespace Utage
{
	...
	public class AssetBundleHelper
	{
		...
		{
			AssetBundleTargetFlags target;
			if (TryGetEditorAssetBundleTargetOverride(out target))
			{
				return target;
			}

			target = BuildTargetToBuildTargetFlag(EditorUserBuildSettings.activeBuildTarget);
			if (target != 0)
			{
				return target;
			}
		...
		const string EditorAssetBundleTargetPrefsKey = "Utage.EditorAssetBundleTarget";

		public static void SetEditorAssetBundleTarget(AssetBundleTargetFlags target)
		{
			EditorPrefs.SetString(EditorAssetBundleTargetPrefsKey, target.ToString());
		}

		public static void ClearEditorAssetBundleTarget()
		{
			EditorPrefs.DeleteKey(EditorAssetBundleTargetPrefsKey);
		}

		public static bool HasEditorAssetBundleTargetOverride()
		{
			AssetBundleTargetFlags target;
			return TryGetEditorAssetBundleTargetOverride(out target);
		}

		static bool TryGetEditorAssetBundleTargetOverride(out AssetBundleTargetFlags target)
		{
			string value = EditorPrefs.GetString(EditorAssetBundleTargetPrefsKey, "");
			...
			try
			{
				target = (AssetBundleTargetFlags)Enum.Parse(typeof(AssetBundleTargetFlags), value, true);
				return target != 0;
			}
			catch
			{
