...
namespace Utage
{
	...
	internal class SoundAudioPlayer : MonoBehaviour
	{
		...
		internal void Remove(SoundAudio audio)
		{
			...
			if (this.Group != null && this.Group.AutoDestoryPlayer && AudioList.Count == 0)
			{
		...
		{
			if (AudioList == null) return false;
		...
		{
			return (Audio != null && IsPlaying()) ? Audio.GetSamplesVolume(): 0;
		}

		public bool IsLoading{ get { return AudioList != null && AudioList.Exists(x => x != null && x.IsLoading); } }
