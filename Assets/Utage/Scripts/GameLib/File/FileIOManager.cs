...
namespace Utage
{
	...
	public class FileIOManager : FileIOManagerBase
	{
		...
		{
			Exception exception;
			bool result = TryReadBinaryDecode(path, callbackRead, out exception);
			if (!result && exception != null)
			{
				Debug.LogError(GetReadErrorMessage(path, exception));
			}
			return result;
		}

		public virtual bool TryReadBinaryDecode(string path, Action<BinaryReader> callbackRead, out Exception exception)
		{
			exception = null;
			try
			{
		...
		protected virtual string GetReadErrorMessage(string path, Exception exception)
		{
			return LanguageErrorMsg.LocalizeTextFormat(ErrorMsg.FileRead, path, exception.ToString());
		}
