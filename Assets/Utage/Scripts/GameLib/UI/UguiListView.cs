...
namespace Utage
{
	...
	public class UguiListView : MonoBehaviour
	{
		...
		bool didWarnMissingContent = false;
		...
		RectTransform ResolveContent()
		{
			...
			if (resolved == null)
			{
				...
				{
					if (rectTransform.transform == transform) continue;
					if (rectTransform.name != "Content") continue;
					resolved = rectTransform;
					break;
				}
			}
			...
			if (resolved == null)
			{
				...
				resolved = go.GetComponent<RectTransform>();
				resolved.anchorMin = Vector2.zero;
				resolved.anchorMax = Vector2.one;
				resolved.offsetMin = Vector2.zero;
				resolved.offsetMax = Vector2.zero;
				if (!didWarnMissingContent)
				{
					Debug.LogWarning("ScrollRect content was missing. Created fallback Content.", this);
					didWarnMissingContent = true;
				}
			}

			scrollRect.content = resolved;
			return resolved;
		}
