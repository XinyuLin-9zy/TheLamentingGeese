...
public class SkipAllOrRead : MonoBehaviour
{
	public UtageUguiConfig config;

	public Button skipAll;

	public Button skipRead;
	...
	public void CheckSkipUnread()
	{
		Initialize();
		AdvEngine engine = config != null ? config.Engine : null;
		if (engine == null) return;

		bool skipUnread = engine.Config.IsSkipUnread;
		SetSelected(skipAll, skipUnread);
		SetSelected(skipRead, !skipUnread);
	}
	...
	private void Initialize()
	{
		...
		if (skipAll == null) skipAll = FindButton("SkipAll", "Skip All");
		if (skipRead == null) skipRead = FindButton("SkipRead", "Skip Read");

		Bind(skipAll, () => SetSkipUnread(true));
		Bind(skipRead, () => SetSkipUnread(false));
	...
	private void SetSkipUnread(bool value)
	{
		AdvEngine engine = config != null ? config.Engine : null;
		if (engine == null) return;

		engine.Config.IsSkipUnread = value;
		CheckSkipUnread();
	}
	...
	private void SubscribeConfig()
	{
		...
		config.OnLoadValues.RemoveListener(CheckSkipUnread);
		config.OnLoadValues.AddListener(CheckSkipUnread);
	}
